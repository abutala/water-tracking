...
class BatteryHistory:
    ...
    def get_average_gradient(self) -> float:
        ...
        diffs = [self.percentages[i] - self.percentages[i+1] 
...
class PowerwallManager:
    ...
    def sanitize_battery_percentage(self, pct: float, time_sampling: float) -> float:
        ...
        if pct != original_pct:
            logging.warning(f"Bad battery data: {original_pct}% -> {pct}% "
    ...
    def get_powerwall_data(self, product) -> dict:
        ...
        if can_export != "battery_ok" or not can_grid_charge:
            raise ValueError(f"Invalid powerwall config - export: {can_export}, grid_charge: {can_grid_charge}")
    ...
    def calculate_trigger_percentages(self, decision_point, current_time, sleep_time: int) -> tuple:
        ...
        trigger_now = round(
            decision_point.pct_thresh - (decision_point.pct_gradient_per_hr * hours_to_end), 2
        )
        trigger_next = round(
    ...
    def apply_decision_point(self, product, data: dict, decision_point, trigger_percentage: float) -> bool:
        ...
        if status_messages and len([msg for msg in status_messages if "Reserve:" in msg]) > 0:
            message = (f"At: {data['battery_percent']}%, {decision_point.reason} - "
    ...
    def process_decision_points(self, product, data: dict, current_time, sleep_time: int) -> int:
        ...
        for decision_point in decision_points:
            ...
            logging.info(f"Evaluating {decision_point.reason}: "
                        f"current={data['battery_percent']:.2f}% vs trigger={trigger_now:.2f}%, "
                        f"future={future_pct:.2f}% vs next_trigger={trigger_next:.2f}%")
            
            # Check if current condition matches
            if self.evaluate_condition(data['battery_percent'], trigger_now, decision_point.iff_higher):
            ...
            # Check if future condition will match (fast retry)
                ...
                logging.warning(f"Future condition match: {decision_point.reason} - fast retry")
        ...
        with Tesla(self.email, verify=False, proxy=None, sso_base_url=None) as tesla:
            ...
            while True:
                ...
                try:
                    ...
                    logging.info(f"Battery: {data['battery_percent']:.2f}%, "
...
def main() -> None:
    ...
    parser.add_argument("-e", "--email", default=Constants.POWERWALL_EMAIL, help="Tesla account email")
